...
void detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis) {
    ...
    // Update result vector with corners
...
void detKeypointsHarris(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis) {
    ...
    for (size_t j = 0; j < dst_norm.rows; j++) {
        for (size_t i = 0; i < dst_norm.cols; i++) {
            ...
            if (response > minResponse) { // Retain points above threshold
                ...
                // Implement local Non-Maximum Suppression (NMS) about the new KP
                bool bOverlap = false;
                for (auto it = keypoints.begin(); it != keypoints.end(); ++it) {
                    ...
                    if (kptOverlap > maxOverlap) {
                        ...
                        if (newKeyPoint.response >
                            (*it).response) {
                            *it = newKeyPoint; // Update KP
                            break;
                        }
                    }
                }
                if (!bOverlap) {
                    keypoints.push_back(newKeyPoint); // Save new KP (VOLATILE)
                }
            }
        } // eof loop over cols
    }     // eof loop over rows
